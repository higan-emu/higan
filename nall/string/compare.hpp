...
inline auto string::_compare(const char* target, uint capacity, const char* source, uint size) -> int {
...
inline auto string::compare(string_view x, string_view y) -> int {
  return memory::compare(x.data(), x.size() + 1, y.data(), y.size() + 1);
}

inline auto string::icompare(string_view x, string_view y) -> int {
  return memory::icompare(x.data(), x.size() + 1, y.data(), y.size() + 1);
}

inline auto string::compare(string_view source) const -> int {
  return memory::compare(data(), size() + 1, source.data(), source.size() + 1);
}

inline auto string::icompare(string_view source) const -> int {
