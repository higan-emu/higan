...
template<uint Precision> struct Integer {
  ...
  template<uint Bits> inline Integer(Integer<Bits> value) { data = value & mask(); }
  template<typename T> inline Integer(const T& value) { data = stype(value) & mask(); }
  explicit inline Integer(const char* value) { data = toInteger(value) & mask(); }

  inline operator stype() const { return data; }

  inline auto operator++(int) { auto value = *this; data = data + 1 & mask(); return value; }
  inline auto operator--(int) { auto value = *this; data = data - 1 & mask(); return value; }

  inline auto& operator++() { data = data + 1 & mask(); return *this; }
  inline auto& operator--() { data = data - 1 & mask(); return *this; }

  template<typename T> inline auto& operator  =(const T& value) { data = stype(        value) & mask(); return *this; }
  template<typename T> inline auto& operator *=(const T& value) { data = stype(data  * value) & mask(); return *this; }
  template<typename T> inline auto& operator /=(const T& value) { data = stype(data  / value) & mask(); return *this; }
  template<typename T> inline auto& operator %=(const T& value) { data = stype(data  % value) & mask(); return *this; }
  template<typename T> inline auto& operator +=(const T& value) { data = stype(data  + value) & mask(); return *this; }
  template<typename T> inline auto& operator -=(const T& value) { data = stype(data  - value) & mask(); return *this; }
  ...
  inline auto operator()(int index) -> BitRange<Precision> { return {(utype&)data, index}; }
  inline auto operator()(int lo, int hi) -> BitRange<Precision> { return {(utype&)data, lo, hi}; }

  inline auto operator()(int index) const -> const BitRange<Precision> { return {(utype&)data, index}; }
  inline auto operator()(int lo, int hi) const -> const BitRange<Precision> { return {(utype&)data, lo, hi}; }

  inline auto bit(int index) -> BitRange<Precision> { return {(utype&)data, index}; }
  ...
  inline auto bit(int index) const -> const BitRange<Precision> { return {(utype&)*this, index}; }
