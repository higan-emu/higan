...
template<uint Precision> struct Natural {
  ...
  template<uint Bits> inline Natural(Natural<Bits> value) { data = value & mask(); }
  template<typename T> inline Natural(const T& value) { data = utype(value) & mask(); }
  explicit inline Natural(const char* value) { data = toNatural(value) & mask(); }
  ...
  template<typename T> inline auto& operator  =(const T& value) { data = utype(        value) & mask(); return *this; }
  template<typename T> inline auto& operator *=(const T& value) { data = utype(data  * value) & mask(); return *this; }
  template<typename T> inline auto& operator /=(const T& value) { data = utype(data  / value) & mask(); return *this; }
  template<typename T> inline auto& operator %=(const T& value) { data = utype(data  % value) & mask(); return *this; }
  template<typename T> inline auto& operator +=(const T& value) { data = utype(data  + value) & mask(); return *this; }
  template<typename T> inline auto& operator -=(const T& value) { data = utype(data  - value) & mask(); return *this; }
  ...
  inline auto bit(int index) const -> const BitRange<Precision> { return {(utype&)data, index}; }
