...
struct VariadicNatural {
  ...
  auto operator++(int) { auto value = data; assign(data + 1); return value; }
  auto operator--(int) { auto value = data; assign(data - 1); return value; }

  auto& operator++() { return assign(data + 1); }
  auto& operator--() { return assign(data - 1); }

  auto& operator &=(const uint64_t value) { return assign(data  & value); }
  auto& operator |=(const uint64_t value) { return assign(data  | value); }
  auto& operator ^=(const uint64_t value) { return assign(data  ^ value); }
  auto& operator<<=(const uint64_t value) { return assign(data << value); }
  auto& operator>>=(const uint64_t value) { return assign(data >> value); }
  ...
  struct Reference {
    Reference(VariadicNatural& self, uint lo, uint hi) : self(self), Lo(lo), Hi(hi) {}
    ...
    auto& operator=(const uint64_t value) {
      const uint64_t RangeBits = Hi - Lo + 1;
      const uint64_t RangeMask = (((1ull << RangeBits) - 1) << Lo) & self.mask;
  ...
  auto bits(uint lo, uint hi) -> Reference { return {*this, lo < hi ? lo : hi, hi > lo ? hi : lo}; }
  auto bit(uint index) -> Reference { return {*this, index, index}; }
  auto byte(uint index) -> Reference { return {*this, index * 8 + 0, index * 8 + 7}; }
