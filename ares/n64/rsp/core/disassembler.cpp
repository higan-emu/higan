...
auto RSP::Disassembler::EXECUTE() -> vector<string> {
  ...
  auto branch  = [&] { return immediate(uint12(address + 4 + (i16(instruction) << 2))); };
  ...
  auto ADDI = [&](string_view add, string_view sub, string_view mov) -> vector<string> {
    if(!(instruction >> 21 & 31)) return {mov, rtName(), immediate(i16(instruction), 32L)};
...
auto RSP::Disassembler::VU() -> vector<string> {
  ...
  auto vdName  = [&] { return vuRegisterName (instruction >>  6 & 31); };
  auto vdValue = [&] { return vuRegisterValue(instruction >>  6 & 31); };
  auto vsName  = [&] { return vuRegisterName (instruction >> 11 & 31); };
  auto vsValue = [&] { return vuRegisterValue(instruction >> 11 & 31); };
  auto vtName  = [&] { return vuRegisterName (instruction >> 16 & 31, instruction >> 21 & 15); };
  auto vtValue = [&] { return vuRegisterValue(instruction >> 16 & 31, instruction >> 21 & 15); };
  auto vmName  = [&] { return vuRegisterName (instruction >>  6 & 31, instruction >> 11 & 31); };
  auto vmValue = [&] { return vuRegisterValue(instruction >>  6 & 31, instruction >> 11 & 31); };
...
auto RSP::Disassembler::immediate(i64 value, uint bits) const -> string {
  if(value < 0) return {"-$", hex(-value, bits >> 2)};
  return {"$", hex(value, bits >> 2)};
};

auto RSP::Disassembler::cpuRegisterName(uint index) const -> string {
...
auto RSP::Disassembler::cpuRegisterValue(uint index) const -> string {
  if(index && showValues) return {cpuRegisterName(index), hint("{$", hex(self.core.r[index].u32, 8L), "}")};
  return cpuRegisterName(index);
}

auto RSP::Disassembler::cpuRegisterIndex(uint index, i16 offset) const -> string {
  string adjust;
  if(offset >= 0) adjust = {"+$", hex( offset)};
  if(offset <  0) adjust = {"-$", hex(-offset)};
  if(index && showValues) return {cpuRegisterName(index), adjust, hint("{$", hex(self.core.r[index].u32 + offset, 8L), "}")};
...
auto RSP::Disassembler::sccRegisterValue(uint index) const -> string {
  u32 value = 0;
  if(index <= 6) value = Nintendo64::rsp.readWord((index & 7) << 2);
...
auto RSP::Disassembler::vuRegisterName(uint index, uint element) const -> string {
