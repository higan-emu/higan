...
auto CPU::Disassembler::SPECIAL() -> vector<string> {
  ...
  case 0x10: return {"mfhi", rdName(), {"hi", hint("{$", hex(self.core.hi.u64, 8L), "}")}};
  case 0x11: return {"mthi", rsValue(), "hi"};
  case 0x12: return {"mflo", rdName(), {"lo", hint("{$", hex(self.core.lo.u64, 8L), "}")}};
...
auto CPU::Disassembler::cpuRegisterName(uint index) const -> string {
...
auto CPU::Disassembler::cpuRegisterValue(uint index) const -> string {
  if(index && showValues) return {cpuRegisterName(index), hint("{$", hex(self.core.r[index].u64, 8L), "}")};
  return cpuRegisterName(index);
}

auto CPU::Disassembler::cpuRegisterIndex(uint index, i16 offset) const -> string {
  string adjust;
  if(offset >= 0) adjust = {"+$", hex( offset)};
  if(offset <  0) adjust = {"-$", hex(-offset)};
  if(index && showValues) return {cpuRegisterName(index), adjust, hint("{$", hex(self.core.r[index].u64 + offset, 8L), "}")};
