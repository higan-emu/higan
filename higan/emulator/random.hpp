...
struct Random {
  ...
  auto array(uint8* data, uint32 size) -> void {
    ...
    for(uint32 address : range(size)) {
      uint8 value = address.field(lobit) ? lovalue : hivalue;
      if(address.field(hibit)) value = ~value;
      if((random() &  511) == 0) value.field(random() & 7) ^= 1;
      if((random() & 2047) == 0) value.field(random() & 7) ^= 1;
      data[address] = value;
    }
  }
