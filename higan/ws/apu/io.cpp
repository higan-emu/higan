auto APU::portRead(uint16 addr) -> uint8 {
  //SDMA_SRC
  if(addr == 0x004a) return dma.s.source.bit( 0, 7);
  if(addr == 0x004b) return dma.s.source.bit( 8,15);
  if(addr == 0x004c) return dma.s.source.bit(16,19);

  //SDMA_LEN
  if(addr == 0x004e) return dma.s.length.bit( 0, 7);
  if(addr == 0x004f) return dma.s.length.bit( 8,15);
  if(addr == 0x0050) return dma.s.length.bit(16,19);
  ...
  //SND_CH1_PITCH
  if(addr == 0x0080) return channel1.r.pitch.bit(0, 7);
  if(addr == 0x0081) return channel1.r.pitch.bit(8,11);

  //SND_CH2_PITCH
  if(addr == 0x0082) return channel2.r.pitch.bit(0, 7);
  if(addr == 0x0083) return channel2.r.pitch.bit(8,11);

  //SND_CH3_PITCH
  if(addr == 0x0084) return channel3.r.pitch.bit(0, 7);
  if(addr == 0x0085) return channel3.r.pitch.bit(8,11);

  //SND_CH4_PITCH
  if(addr == 0x0086) return channel4.r.pitch.bit(0, 7);
  if(addr == 0x0087) return channel4.r.pitch.bit(8,11);
  ...
  //SND_RANDOM
  if(addr == 0x0092) return channel4.s.noiseLFSR.bit(0, 7);
  if(addr == 0x0093) return channel4.s.noiseLFSR.bit(8,14);
...
auto APU::portWrite(uint16 addr, uint8 data) -> void {
  //SDMA_SRC
  if(addr == 0x004a) dma.r.source.bit( 0, 7) = data.bit(0,7);
  if(addr == 0x004b) dma.r.source.bit( 8,15) = data.bit(0,7);
  if(addr == 0x004c) dma.r.source.bit(16,19) = data.bit(0,3);

  //SDMA_LEN
  if(addr == 0x004e) dma.r.length.bit( 0, 7) = data.bit(0,7);
  if(addr == 0x004f) dma.r.length.bit( 8,15) = data.bit(0,7);
  if(addr == 0x0050) dma.r.length.bit(16,19) = data.bit(0,3);
  ...
  //SND_CH1_PITCH
  if(addr == 0x0080) channel1.r.pitch.bit(0, 7) = data.bit(0,7);
  if(addr == 0x0081) channel1.r.pitch.bit(8,11) = data.bit(0,3);

  //SND_CH2_PITCH
  if(addr == 0x0082) channel2.r.pitch.bit(0, 7) = data.bit(0,7);
  if(addr == 0x0083) channel2.r.pitch.bit(8,11) = data.bit(0,3);

  //SND_CH3_PITCH
  if(addr == 0x0084) channel3.r.pitch.bit(0, 7) = data.bit(0,7);
  if(addr == 0x0085) channel3.r.pitch.bit(8,11) = data.bit(0,3);

  //SND_CH4_PITCH
  if(addr == 0x0086) channel4.r.pitch.bit(0, 7) = data.bit(0,7);
  if(addr == 0x0087) channel4.r.pitch.bit(8,11) = data.bit(0,3);
  ...
  //SND_SWEEP_TIME
  if(addr == 0x008d) channel3.r.sweepTime = data.bit(0,4);
