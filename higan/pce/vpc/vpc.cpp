...
auto VPC::bus(uint hclock) -> uint9 {
  ...
  auto enableVDC0 = settings[mode].enableVDC0 && bus0.bit(0,3);
  auto enableVDC1 = settings[mode].enableVDC1 && bus1.bit(0,3);
...
auto VPC::read(uint5 addr) -> uint8 {
  ...
  if(addr == 0x0a) return window[0].bit(0,7);
  if(addr == 0x0b) return window[0].bit(8,9);
  if(addr == 0x0c) return window[1].bit(0,7);
  if(addr == 0x0d) return window[1].bit(8,9);
