auto YM2413::Voice::update(const uint8* data) -> void {
  if(data) {
    modulator.multiple    = data[0].bits(0,3);
  ...
  //unused                    = data[3].bit (5);
    ...
    modulator.decay  = data[4].bits(0,3);
    modulator.attack = data[4].bits(4,7);

    carrier.decay  = data[5].bits(0,3);
    carrier.attack = data[5].bits(4,7);

    modulator.release = data[6].bits(0,3);
    modulator.sustain = data[6].bits(4,7);

    carrier.release = data[7].bits(0,3);
    carrier.sustain = data[7].bits(4,7);
  }
