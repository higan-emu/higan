auto V9938::status() -> uint8 {
  ...
  case 8:
    data.bits(0,7) = op.match.bits(0,7);
    return data;
...
auto V9938::data() -> uint8 {
  io.controlLatch = 0;
  uint17 address = io.ramBank << 14 | io.controlValue.bits(0,13)++;
  if(!io.controlValue.bits(0,13)) io.ramBank++;  //unconfirmed
...
auto V9938::control(uint8 data) -> void {
  ...
  if(io.controlValue.bit(15)) {
    return register(io.controlValue.bits(8,13), io.controlValue.bits(0,7));
  }
...
auto V9938::palette(uint8 data) -> void {
  ...
  paletteRAM[io.paletteIndex].bits(0,2) = io.paletteValue.bits(0, 2);  //B
  paletteRAM[io.paletteIndex].bits(3,5) = io.paletteValue.bits(4, 6);  //R
  paletteRAM[io.paletteIndex].bits(6,8) = io.paletteValue.bits(8,10);  //G
  io.paletteIndex++;
}
...
auto V9938::register(uint6 register, uint8 data) -> void {
  ...
  case 0x02:
    table.patternLayout.bits(10,16) = data.bits(0,6);
    return;

  case 0x03:
    table.color.bits(6,13) = data.bits(0,7);
    return;

  case 0x04:
    table.patternGenerator.bits(11,16) = data.bits(0,5);
    return;

  case 0x05:
    table.spriteAttribute.bits(7,14) = data.bits(0,7);
    return;

  case 0x06:
    table.spritePatternGenerator.bits(11,16) = data.bits(0,5);
    return;
  ...
  case 0x0a:
    table.color.bits(14,16) = data.bits(0,2);
    return;

  case 0x0b:
    table.spriteAttribute.bits(15,16) = data.bits(0,1);
    return;

  case 0x0c:
    io.blinkColorBackground = data.bits(0,3);
    io.blinkColorForeground = data.bits(4,7);
    return;

  case 0x0d:
    io.blinkPeriodBackground = data.bits(0,3);
    io.blinkPeriodForeground = data.bits(4,7);
    return;

  case 0x0e:
    io.ramBank = data.bits(0,2);
    return;

  case 0x0f:
    io.statusIndex = data.bits(0,3);
    return;

  case 0x10:
    io.paletteIndex = data.bits(0,3);
    return;

  case 0x11:
    io.registerIndex = data.bits(0,5);
    io.registerFixed = data.bit(7);
    return;

  case 0x12:
    screen.hadjust = data.bits(0,3);
    screen.vadjust = data.bits(4,7);
    return;
  ...
  case 0x20: op.sx.bits(0,7) = data.bits(0,7); return;
  case 0x21: op.sx.bit (8)   = data.bit (0);   return;
  case 0x22: op.sy.bits(0,7) = data.bits(0,7); return;
  case 0x23: op.sy.bits(8,9) = data.bits(0,1); return;
  case 0x24: op.dx.bits(0,7) = data.bits(0,7); return;
  case 0x25: op.dx.bit (8)   = data.bit (0);   return;
  case 0x26: op.dy.bits(0,7) = data.bits(0,7); return;
  case 0x27: op.dy.bits(8,9) = data.bits(0,1); return;
  case 0x28: op.nx.bits(0,7) = data.bits(0,7); return;
  case 0x29: op.nx.bit (8)   = data.bit (0);   return;
  case 0x2a: op.ny.bits(0,7) = data.bits(0,7); return;
  case 0x2b: op.ny.bits(8,9) = data.bits(0,1); return;
  case 0x2c: op.cr.bits(0,7) = data.bits(0,7); op.ready = 0; return;
