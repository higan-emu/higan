auto Cartridge::MBC6::read(uint16 address) -> uint8 {
  ...
  if((address & 0xe000) == 0x4000) {  //$4000-5fff
    return cartridge.rom.read(io.rom.bank[0] << 13 | address(0,12));
  }

  if((address & 0xe000) == 0x6000) {  //$6000-7fff
    return cartridge.rom.read(io.rom.bank[1] << 13 | address(0,12));
  }

  if((address & 0xf000) == 0xa000) {  //$a000-afff
    if(!io.ram.enable) return 0xff;
    return cartridge.ram.read(io.ram.bank[0] << 12 | address(0,11));
  }

  if((address & 0xf000) == 0xb000) {  //$b000-bfff
    if(!io.ram.enable) return 0xff;
    return cartridge.ram.read(io.ram.bank[1] << 12 | address(0,11));
  }
...
auto Cartridge::MBC6::write(uint16 address, uint8 data) -> void {
  ...
  if((address & 0xf000) == 0xa000) {  //$a000-afff
    ...
    return cartridge.ram.write(io.ram.bank[0] << 12 | address(0,11), data);
  }

  if((address & 0xf000) == 0xb000) {  //$b000-bfff
    if(!io.ram.enable) return;
    return cartridge.ram.write(io.ram.bank[1] << 12 | address(0,11), data);
  }
}
