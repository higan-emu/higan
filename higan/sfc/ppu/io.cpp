...
auto PPU::writeOAM(uint10 address, uint8 data) -> void {
  if(!io.displayDisable && vcounter() < vdisp()) {
    if(address.bit(9) == 0) obj.oam.write(0x000 | latch.oamAddress << 2 | address & 1, data);
    if(address.bit(9) == 1) obj.oam.write(0x200 | latch.oamAddress >> 2, data);
    return;
  }
...
auto PPU::writeIO(uint24 address, uint8 data) -> void {
  ...
  case 0x2106: {
    ...
    Background::Mosaic::size = data.bit(4,7);
    return;
  }
  ...
  //COLDATA
  case 0x2132: {
    if(data.bit(5)) screen.io.colorRed   = data.bit(0,4);
    if(data.bit(6)) screen.io.colorGreen = data.bit(0,4);
    if(data.bit(7)) screen.io.colorBlue  = data.bit(0,4);
    return;
  }
