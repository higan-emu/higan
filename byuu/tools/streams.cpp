auto StreamManager::construct() -> void {
  ...
  streamList.onToggle([&](auto cell) {
    if(auto item = streamList.selected()) {
      if(auto stream = item.attribute<higan::Node::Stream>("node")) {
...
auto StreamManager::reload() -> void {
  ...
  for(auto stream : higan::Node::enumerate<higan::Node::Stream>(emulator->root)) {
    TableViewItem item{&streamList};
    item.setAttribute<higan::Node::Stream>("node", stream);
