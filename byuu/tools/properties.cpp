...
auto PropertiesViewer::reload() -> void {
  ...
  for(auto properties : higan::Node::enumerate<higan::Node::Properties>(emulator->root)) {
    ComboButtonItem item{&propertiesList};
    item.setAttribute<higan::Node::Properties>("node", properties);
...
auto PropertiesViewer::refresh() -> void {
  if(auto item = propertiesList.selected()) {
    if(auto properties = item.attribute<higan::Node::Properties>("node")) {
