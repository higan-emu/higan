...
auto MemoryEditor::reload() -> void {
  memoryList.reset();
  for(auto memory : higan::Node::enumerate<higan::Node::Memory>(emulator->root)) {
    ComboButtonItem item{&memoryList};
    item.setAttribute<higan::Node::Memory>("node", memory);
...
auto MemoryEditor::eventChange() -> void {
  if(auto item = memoryList.selected()) {
    if(auto memory = item.attribute<higan::Node::Memory>("node")) {
