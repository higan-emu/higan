...
auto GraphicsViewer::reload() -> void {
  graphicsList.reset();
  for(auto graphics : higan::Node::enumerate<higan::Node::Graphics>(emulator->root)) {
    ComboButtonItem item{&graphicsList};
    item.setAttribute<higan::Node::Graphics>("node", graphics);
...
auto GraphicsViewer::refresh() -> void {
  if(auto item = graphicsList.selected()) {
    if(auto graphics = item.attribute<higan::Node::Graphics>("node")) {
