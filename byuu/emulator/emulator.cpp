...
auto Emulator::manifest() -> shared_pointer<vfs::file> {
  if(auto cartridge = medium.cast<icarus::Cartridge>()) {
...
//it is a fallback when icarus is unable to generate manifests for game CD-ROMs
...
auto Emulator::manifest(const string& type, const string& location) -> shared_pointer<vfs::file> {
  if(auto medium = icarus::medium(type)) {
...
auto Emulator::setOverscan(bool value) -> bool {
  if(auto screen = root->scan<higan::Node::Screen>("Screen")) {
    if(auto overscan = screen->find<higan::Node::Boolean>("Overscan")) {
